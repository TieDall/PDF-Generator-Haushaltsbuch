    ...
    public static void generateBarchart() throws IOException{
        ...
        for (Iterator<GetBarDataResponse.DataBar.Entry> iterator = entry.iterator(); iterator.hasNext();) {
            ...
            if (next.getKey().equals("Einnahmen")) {
                chart.addSeries("Einnahme", Arrays.asList(new Integer[] { 0}), Arrays.asList(new Float[] { next.getValue()})).setFillColor(Color.green);
            } else if (next.getKey().equals("Ausgaben")) {
                chart.addSeries("Ausgabe", Arrays.asList(new Integer[] { 1}), Arrays.asList(new Float[] { next.getValue()})).setFillColor(Color.red);
            }
        }        
            
        BitmapEncoder.saveBitmap(chart, "C:/xampp/htdocs/Barchart", BitmapEncoder.BitmapFormat.PNG);
